#ifndef __PLATFORM_STM32_FLASH_H__
#define __PLATFORM_STM32_FLASH_H__
 ...
 * 1、对于同一个地址空间，仅支持一次编程(不推荐二次编程，即使是将相应 bit 由数值 1 编程 0)。
   ...
   3、H7 的 Flash 编程时，务必保证要编程的地址是 32 字节对齐的，即此地址对 32 求余为 0。并且编
    程的数据必须 32 字节整数倍。
 *
 * @param FlashAddress 要擦除的FLASH地址
 * @return bool 如果擦除成功返回true，否则返回false
 */
 ...
 * 1、对于同一个地址空间，仅支持一次编程(不推荐二次编程，即使是将相应 bit 由数值 1 编程 0)。
   2、只能对已经擦除的空间做编程，擦除 1 个扇区是 128KB。
   3、H7 的 Flash 编程时，务必保证要编程的地址是 32 字节对齐的，即此地址对 32 求余为 0。并且编
    程的数据必须 32 字节整数倍。该函数Platform_Flash_EraseCpuFlash对字节数不够 32 字节整数倍的情况自动补0。
 *
 * @param FlashAddress 要写入的FLASH地址
 ...
 * 1、对于同一个地址空间，仅支持一次编程(不推荐二次编程，即使是将相应 bit 由数值 1 编程 0)。
   ...
   3、H7 的 Flash 编程时，务必保证要编程的地址是 32 字节对齐的，即此地址对 32 求余为 0。并且编
    程的数据必须 32 字节整数倍。
 *
 * @param FlashAddress 要读取的FLASH地址
...
#endif //__PLATFORM_STM32_FLASH_H__
